...
impl EHPR {
    fn new(input: &Vec<i64>, (height, width): (usize, usize)) -> Self {
        EHPR {
            loc: (0, 0),
            dir: Direction::Up,
            board: vec![vec![BLACK.into(); width]; height],
    ...
    fn tick(&mut self) -> bool {
        match self.comp.run() {
            ...
            _ => panic!("Opcode failed to parse!"),
        }
...
fn part1(inp: &Vec<i64>) -> i64 {
    ...
    // originally done with a 10000x10000 grid with the robot starting
    // at (5000,5000), it was reduced post-facto to get this to run faster,
    // admittedly Rust is already quite full-threaded and memory-hungry

    let mut robot = EHPR::new(&inp, (192, 192));
    robot.set_location((96, 96));
